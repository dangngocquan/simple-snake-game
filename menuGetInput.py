...
class GetInputStringMenu:
    ...
    def __init__(self, x, y, width, height,
                 ...
                 textWhenIncorrect="Your name is invalid. Please try again.",
                 textHint="~"):
        ###########  Surface, cursor   ######################################################################
        self.surface = pygame.Surface((width, height), pygame.SRCALPHA)
        self.surfaceRect = self.surface.get_rect()
        ...
        for i in range(1):
            cell = pygame.Surface(
                (self.container1Rect.width//1 - 20, self.container1Rect.height - 40), 
                pygame.SRCALPHA)
            cellRect = cell.get_rect()
            cellRect.topleft = (10 + self.container1Rect.width//1*i, 20)
            self.cells.append([cell, cellRect])
        ...
        self.cursor = 0
        ########### Buttons   ###############################################################################
        self.titlePassword = Button("YOUR NAME", MEDIUM_FONT, width//2, 20)
        self.titlePassword.isChosen = True
        self.titleCancer = Button("CANCER", MEDIUM_FONT_2, width//4, height-20)
        self.titleEnter = Button("ENTER", MEDIUM_FONT_2, width//4*3, height-20)
